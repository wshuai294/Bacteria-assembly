...
def run_prokka(fasta, outdir, prefix, cpus):
    cmd = f"""prokka --quiet --force --cpus {cpus} --outdir {outdir}  --prefix {prefix} {fasta}"""
    os.system(cmd)

def get_gene_graph(ref, seg_fa, outdir, ID, cpus): # graph constructed by the gene distance in the original ref
